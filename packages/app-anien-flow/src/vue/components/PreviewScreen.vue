...
const activeStrip = computed<StripVM | null>(() => {
  const currentTick = stateService.currentTick();
  return (
    stateService
      .timelineItems()
...
const activeMedia = computed<ScreenMediaViewModel | null>(() => {
  const strip = activeStrip.value;
  if (!strip) {
    return null;
  }

  const sourceMetadata = stateService.debugSnapshot()?.stripSources[strip.sourceId]?.metadata;
  if (!sourceMetadata || typeof sourceMetadata !== 'object') {
    return null;
  }

  const metadata = sourceMetadata as Record<string, unknown>;
  const url = resolveMediaUrl(metadata);
...
function resolveMediaUrl(metadata: Record<string, unknown>): string | null {
  if (typeof metadata['uploadedFileUrl'] === 'string' && metadata['uploadedFileUrl'].length > 0) {
    const normalizedUrl = normalizeUploadedFileUrl(metadata['uploadedFileUrl']);
    return normalizedUrl ?? metadata['uploadedFileUrl'];
  }

  if (typeof metadata['uploadedFilePath'] === 'string' && metadata['uploadedFilePath'].length > 0) {
...
function normalizeUploadedFileUrl(uploadedFileUrl: string): string | null {
  ...
  <section class="screen-root" aria-label="Preview screen">
    <template v-if="activeMedia">
      ...
      <video
        ...
        class="screen-video"
        :src="activeMedia.url"
        :aria-label="activeMedia.sourceName"
        controls
      >
        Your browser does not support video playback.
      </video>
    </template>
    <div v-else class="screen-empty" role="status" aria-live="polite">
